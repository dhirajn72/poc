...
public class BrokerEmail {
	...
	public BrokerEmail(List<String> destinataires, List<String> destinatairesCC, String messageObject,
			String messageBody) {
	...
	public void sendSTMPEmail(InputStream input) throws AddressException, MessagingException {
		...
		try {
			...
			transport.connect(mailserverProperties.getProperty("username"),
					mailserverProperties.getProperty("password"));
			transport.sendMessage(myEmailMessage, myEmailMessage.getAllRecipients());
			transport.close();
			System.out.println("===> Done !");

		} catch (FileNotFoundException ex) {
			Logger.getLogger(BrokerEmail.class.getName()).log(Level.SEVERE, null, ex);
